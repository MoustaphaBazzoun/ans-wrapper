...
from ans_wrapper.enums import STATE_CODES, BRAZILIAN_STATE_CODES
...
class Beneficiarios:
    ...
    def build_dataset(
        ...
        if target_date and (start or end):
            raise ValueError("provide either `target_date` or both `start` and `end`, not both.")
        ...
        for state in states:
            if state not in BRAZILIAN_STATE_CODES:
                raise ValueError(f"invalid state: {state}, allowed states are: {BRAZILIAN_STATE_CODES}")
